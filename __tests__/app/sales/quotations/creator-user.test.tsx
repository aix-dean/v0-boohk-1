...
function TestCreatorComponent({ quotation }) {
...
describe("useEffect - Creator User Fetch Logic", () => {
  ...
  it("fetches and sets creator user when quotation.created_by exists", async () => {
    ...
    await act(async () => {
      render(<TestCreatorComponent quotation={{ created_by: "user123" }} />)
      await flushPromises()
    })
  ...
  it("does not call getUserById when quotation.created_by is missing", async () => {
    await act(async () => {
      render(<TestCreatorComponent quotation={{}} />)
      await flushPromises()
    })
  ...
  it("handles fetch error gracefully", async () => {
    ...
    await act(async () => {
      render(<TestCreatorComponent quotation={{ created_by: "user999" }} />)
      await flushPromises()
    })
